 ...
 */
bool ROSLaunchIndenter::isElectricCharacter(const QChar &ch) const
{
    return ch == ':';
}

int ROSLaunchIndenter::indentFor(const QTextBlock &block, const TextEditor::TabSettings &tabSettings)
{
...
bool ROSLaunchIndenter::isElectricLine(const QString &line) const
{
    if (line.isEmpty())
        return false;
...
int ROSLaunchIndenter::getIndentDiff(const QString &previousLine,
