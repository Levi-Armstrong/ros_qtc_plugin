...
/**
 * @class ROSLaunchEditor::Internal::ROSLaunchHighlighter
...
ROSLaunchHighlighter::ROSLaunchHighlighter()
{
    setTextFormatCategories(Format_FormatsAmount, styleForFormat);
}

/**
 * @brief ROSLaunchHighlighter::highlightBlock highlights single line of Python code
 ...
 */
void ROSLaunchHighlighter::highlightBlock(const QString &text)
{
 ...
 */
int ROSLaunchHighlighter::highlightLine(const QString &text, int initialState)
{
 ...
 */
void ROSLaunchHighlighter::highlightImport(Scanner &scanner)
{
    FormatToken tk;
    while (!(tk = scanner.read()).isEndOfBlock()) {
        Format format = tk.format();
